...
MongoStorage = (function() {
  ...
  MongoStorage.prototype.upload = function(req, res, callback) {
    var busboy, self;
    self = this;
    var _id = new ObjectID(); //Added line
    busboy = new Busboy({
      headers: req.headers
    });
  ...
  MongoStorage.prototype.removeFile = function(id, callback) {
    var _id = undefined
    if(/^[0-9a-fA-F]{24}$/.test(id)) {
      _id = new ObjectID(id);
    } else {
      err = new Error(id+' is not a valid ObjectID');
      err.status = 422;
      return callback(err);
    }
  ...
  MongoStorage.prototype.getFile = function(id, callback) {
    var _id = undefined
    if(/^[0-9a-fA-F]{24}$/.test(id)) {
      _id = new ObjectID(id);
    } else {
  ...
  MongoStorage.prototype.existsId = function(id,callback){
    var _id = undefined
    if(/^[0-9a-fA-F]{24}$/.test(id)) {
      _id = new ObjectID(id);
    } else {
  ...
  MongoStorage.prototype.download = function(id, res, callback) {
    ...
    self = this;
    return this.getFile(id, function(err, file) {
      var gfs, read;
      if (err) {
        return callback(err);
      }
